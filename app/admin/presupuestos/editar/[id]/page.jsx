...
// Asegura que cada item tenga un id único: repara presupuestos guardados
// antes de que existiera este campo, o con ids duplicados por algún guardado
// previo defectuoso (si no, los borrados/ediciones por id afectan más de un
// item a la vez).
const normalizarItems = (items) => {
  const vistos = new Set();
  return (items || []).map((item) => {
    if (item.id === undefined || item.id === null || vistos.has(item.id)) {
...
export default function EditarPresupuesto({ params }) {
  ...
  const addItem = () => {
    setPresupuesto(prev => {
      const newItems = [
        ...prev.items,
        { id: crypto.randomUUID(), descripcion: '', cantidad: 1, precioUnitario: 0, subtotal: 0 }
      ];
