...
export default function VerMantenimientoPreventivo({ params }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const mantenimientoDoc = doc(db, 'mantenimientosPreventivos', id);
        const mantenimientoSnapshot = await getDoc(mantenimientoDoc);

        if (mantenimientoSnapshot.exists()) {
          setMantenimiento({ id: mantenimientoSnapshot.id, ...mantenimientoSnapshot.data() });
        } else {
          alert('Mantenimiento preventivo no encontrado.');
        ...
        alert(
          error.code === 'permission-denied'
            ? 'No tenés permiso para ver este mantenimiento preventivo (no fue creado por vos).'
  ...
  const handleEliminar = async () => {
    if (confirm('¿Está seguro de que desea eliminar este mantenimiento preventivo? También se borrarán sus fotos.')) {
      try {
        ...
        alert('Mantenimiento preventivo eliminado exitosamente.');
        router.push('/admin/mantenimiento-preventivo');
      } catch (error) {
        console.error('Error al eliminar el mantenimiento preventivo:', error);
        alert('Error al eliminar el mantenimiento preventivo.');
      }
    }
  };
      ...
      <div className="container px-4 py-8 mx-auto">
        <div className="max-w-4xl mx-auto bg-white shadow-lg">
          ...
          <div className="px-8 py-4">
            <h1 className="text-xl font-bold text-center text-blue-800">MANTENIMIENTO PREVENTIVO</h1>
          </div>
            ...
            <div className="flex-1 p-4 rounded bg-gray-50">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex">
                  ...
                  <span className="flex-1">{mantenimiento.numero || ''}</span>
                </div>
                <div className="flex">
                  <span className="w-20 font-bold">Fecha:</span>
                  <span className="flex-1">{formatearFecha(mantenimiento.fecha)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 p-4 rounded bg-gray-50">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex">
                  ...
                  <span className="flex-1">{mantenimiento.cliente?.nombre || ''}</span>
                </div>
                <div className="flex">
                  <span className="w-20 font-bold">Empresa:</span>
                  <span className="flex-1">{mantenimiento.cliente?.empresa || ''}</span>
                </div>
                <div className="flex">
                  <span className="w-20 font-bold">Dirección:</span>
                  <span className="flex-1">{mantenimiento.cliente?.direccion || ''}</span>
                </div>
                {mantenimiento.cliente?.sedeNombre && (
                  <div className="flex">
                    <span className="w-20 font-bold">Sede:</span>
                    <span className="flex-1">{mantenimiento.cliente.sedeNombre}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="px-8 py-4">
            ...
            {esHtmlEnriquecido(mantenimiento.descripcionTrabajo) ? (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: mantenimiento.descripcionTrabajo }}
              />
            ) : (
            ...
            <div className="flex flex-col items-center w-2/5">
              {mantenimiento.firmaTecnico && (
                <div className="flex items-center justify-center w-40 h-16 mb-2 border border-gray-200 rounded bg-gray-50">
                  <img src={mantenimiento.firmaTecnico} alt="Firma del técnico" className="object-contain max-w-full max-h-full" />
                </div>
              )}
              <div className="w-full pt-2 border-t border-gray-800">
                <div className="text-xs text-center">Firma del técnico</div>
                <div className="mt-1 text-xs font-bold text-center">{mantenimiento.aclaracionFirmaTecnico || 'Sin aclaración'}</div>
              </div>
            </div>
            <div className="flex flex-col items-center w-2/5">
              {mantenimiento.firmaCliente && (
                <div className="flex items-center justify-center w-40 h-16 mb-2 border border-gray-200 rounded bg-gray-50">
                  <img src={mantenimiento.firmaCliente} alt="Conformidad del cliente" className="object-contain max-w-full max-h-full" />
                </div>
              )}
              <div className="w-full pt-2 border-t border-gray-800">
                <div className="text-xs text-center">Conformidad del cliente</div>
                <div className="mt-1 text-xs font-bold text-center">{mantenimiento.aclaracionFirmaCliente || 'Sin aclaración'}</div>
              </div>
            </div>
          </div>

          {/* Planillas de inspección adjuntas */}
          {mantenimiento.planillasAdjuntas?.length > 0 && (
            <div className="px-8 py-4">
              <h3 className="p-2 mb-3 text-sm font-bold text-blue-800 bg-gray-100 rounded">
                Se adjunta la siguiente inspección
              </h3>
              <div className="space-y-4">
                {mantenimiento.planillasAdjuntas.map((planilla, i) => (
